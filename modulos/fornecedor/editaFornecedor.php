...
$sql = "UPDATE `tb_fornecedor` SET `rsocial` = ?,`nfantasia` = ?,`cnpj` = ?,`descricao`= ?,`estado`= ? , `cidade`= ?, `cep`=?, `bairro`=?, `rua`=?, `numero`=?, `complemento`=?, `representante`=?, `telefone`=?, `celular`=?, `site`=?, `email`=?, `banco`=?, `agencia`=?, `conta`=? WHERE cnpj = ?";
...
if (!$resp) {
    ...
    echo "<a href='index.php'>Voltar aos fornecedores</a><br/>";
